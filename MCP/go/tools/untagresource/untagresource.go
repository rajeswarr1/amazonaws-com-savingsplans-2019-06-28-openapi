...
func UntagresourceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/UntagResource", cfg.BaseURL)
...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_UntagResource",
		mcp.WithDescription("Removes the specified tags from the specified resource."),
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description("Input parameter: The Amazon Resource Name (ARN) of the resource.")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description("Input parameter: The tag keys.")),
	)
